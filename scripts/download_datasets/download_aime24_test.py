"""Generate an AIME24 test sqlite file using TestSetEntry payloads."""
...
def main() -> None:
    parser = create_parser("aime24")
    args = parser.parse_args()

    repo_root = Path(__file__).resolve().parents[2]
    install_hf_token(repo_root)

    dataset = load_aime24_test()
    output_path = args.output or (repo_root / "datasets" / "aime24_test.sqlite")
