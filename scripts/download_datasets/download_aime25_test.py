"""Generate an AIME25 test sqlite file using TestSetEntry payloads."""
...
def main() -> None:
    parser = create_parser("aime25")
    args = parser.parse_args()

    repo_root = Path(__file__).resolve().parents[2]
    install_hf_token(repo_root)

    dataset = load_aime25_test()
    output_path = args.output or (repo_root / "datasets" / "aime25_test.sqlite")
