# Uniform-weight ablation: identical to qwen25_notool_control_pos.sh (same
# pinned questions via question_rotation_seed=0, same LR/budgets) except the
# loss binarizes positive advantages (uniform_positive_advantage=true).
# Compare pinned-rollout curves against notool_control_pos to isolate the
# value of credit-magnitude weighting over plain rejection sampling.
uv run -m src_py.modal.launch_modal_orchestration \
    --model-cli-name qwen25 \
    --max-rollout-concurrency 300 \
    --config-nickname notool_control_uniform \
    ...
    --training-config-common-path config/training/common_lora_control_uniform.toml \
    --training-time 2400 \
