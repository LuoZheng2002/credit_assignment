...
CONFIG_PATH = Path("src_py/modal/oneshot_generation_config.json")
...
def _oneshot_generation_app_name(model_cli_name: str, config_nickname: str) -> str:
    return f"credit-assignment-oneshot-generation-{model_cli_name}-{config_nickname}"


def _write_oneshot_generation_config(
    repo_root: Path,
    cli_args: list[str],
    ...
    if not cli_args:
        raise RuntimeError(
            "No oneshot generation arguments provided. Pass the same flags you would pass to bin_oneshot_generation."
        )
...
def _extract_num_gpus(cli_args: list[str]) -> int:
    ...
    while index < len(cli_args):
        ...
        raise RuntimeError(
            "Missing required --num-gpus argument. Ensure oneshot generation script passes --num-gpus."
        )
...
def _strip_modal_only_args(cli_args: list[str]) -> list[str]:
    ...
    if removed_compute_backend:
        print(
            "Ignoring legacy --compute-backend flag; oneshot generation now always uses local wrapper-managed runtime paths.",
            flush=True,
        )
    if removed_gpu_name:
        print(
            "Stripped Modal-only --gpu-name flag before invoking bin_oneshot_generation.",
            flush=True,
        )
    if removed_modal_time_limit_hrs:
        print(
            "Stripped Modal-only --modal-time-limit-hrs flag before invoking bin_oneshot_generation.",
            flush=True,
        )
    return normalized
...
def _launch_oneshot_generation(repo_root: Path):
    repo_root_str = str(repo_root)
    if repo_root_str not in sys.path:
        sys.path.insert(0, repo_root_str)

    generation_module = importlib.import_module(
        "src_py.modal.modal_oneshot_generation_app"
    )
    app = getattr(generation_module, "app")
    service_cls = getattr(generation_module, "OneshotGenerationService")

    with modal.enable_output():
        with app.run(detach=True):
            instance = service_cls()
            return instance.run_generation.spawn()
...
def main() -> int:
    ...
    service_state_volume_name_generation = experiment_service_state_volume_name(
        model_cli_name, config_nickname_generation, pipeline="generation"
    )
    app_name = _oneshot_generation_app_name(
        model_cli_name, config_nickname_generation
    )
    repo_root = _repo_root()
    print(f"Validated oneshot generation num_gpus: {num_gpus}", flush=True)
    print(f"Validated Modal gpu_name: {gpu_name}", flush=True)
    print(f"Modal time limit: {modal_time_limit_hrs} hrs", flush=True)
    print(
        "Validated oneshot generation runtime: local wrapper-managed inference/training",
        flush=True,
    )
    print(
        f"Resolved generation volume name: {service_state_volume_name_generation}",
        flush=True,
    )
    ...
    with _config_file_lock(repo_root):
        try:
            config_path = _write_oneshot_generation_config(
                repo_root,
                cli_args,
            ...
            print(f"Wrote oneshot generation config: {config_path}", flush=True)
            print(
                f"Submitting Modal oneshot generation app in detached mode: {app_name}",
                flush=True,
            )
            generation_call = _launch_oneshot_generation(repo_root)
        finally:
            _remove_config_file(config_path)
            print(f"Removed oneshot generation config: {config_path}", flush=True)

    if generation_call is None:
        raise RuntimeError("failed to submit Modal oneshot generation call")

    call_id = getattr(generation_call, "object_id", None)
    if call_id is not None:
        print(f"Submitted oneshot generation call id: {call_id}", flush=True)
    else:
        print("Submitted oneshot generation call", flush=True)

    print(
        "The launcher has exited after successfully submitting the oneshot generation job; "
