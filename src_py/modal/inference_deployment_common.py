...
def deployment_name(
    ...
    base = f"modal_inferece_{combined_config_name(model_cli_name, model_api_name, config_nickname, epoch)}"
    return base[:63]


def _deploy_config_path() -> Path:
    return Path(__file__).with_name("inference_deploy_config.json")
...
def load_materialized_deploy_config() -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(
            "missing src_py/modal/inference_deploy_config.json; deploy via wrapper-managed deploy path"
        ) from error
...
def ensure_deployed(
    ...
    try:
        result = subprocess.run(
            ["uv", "run", "modal", "deploy", "modal_inference_app.py", "--name", app_name],
            check=False,
        )
