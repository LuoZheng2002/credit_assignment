"""Shared Pydantic models and CLI utilities for training configuration.

This module is deliberately kept outside ``src_py/train/`` so that the
training wrapper (``src_py.wrappers.training_wrapper``) can import these
models without pulling in ``torch`` (via ``src_py/train/__init__.py``).
"""
