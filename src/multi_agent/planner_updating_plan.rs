...
fn get_planner_updating_plan_prompt_before_assistant(session_state: &SessionState) -> String{
    ...
    let current_step_summary = session_state.current_step_content_compacted.clone().expect("When updating the plan, the compacted current step must be available.");
    format!("\
...
\n\
Please provide the entire updated plan, not just the modified parts, in a markdown code block like the following example:\n\
```\n\
- [x] Step 1: xxx\n\
- [ ] Step 2: xxx\n\
- [ ] Step 3: xxx\n\
