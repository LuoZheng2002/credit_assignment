...
use crate::training_job_client::TrainingJobClient;

#[derive(Clone, Debug)]
pub struct ModalTrainingClient {
...
pub struct ModalTrainStartRequest {
...
#[derive(Clone, Debug, Deserialize)]
pub struct ModalTrainStatusResponse {
    pub status: String,
    pub progress_message: Option<String>,
    pub progress_fraction: Option<f32>,
}

impl ModalTrainingClient {
    pub fn new(
        client: reqwest::Client,
        base_url: &str,
        ...
        if base_url.is_empty() {
            return Err("Modal training base URL cannot be empty".to_string());
        }

        let bearer_token = if let Some(env_var) = auth_token_env_var {
            let env_var = env_var.trim();
            if env_var.is_empty() {
                return Err("Modal auth token env var name cannot be empty".to_string());
            }
            let token = std::env::var(env_var)
                .map_err(|_| format!("Modal auth token env var is not set: {}", env_var))?;
            let token = token.trim().to_string();
            if token.is_empty() {
                return Err(format!("Modal auth token env var is empty: {}", env_var));
            }
            Some(token)
        } else {
            None
    ...
    pub async fn upload_trajectory_file(&self, job_id: &str, file_path: &str) -> Result<(), String> {
        ...
        if !path.is_file() {
            return Err(format!(
                "Training trajectory file does not exist for Modal upload: {}",
                file_path
            ));
        }
        let bytes = tokio::fs::read(path).await.map_err(|err| {
            format!(
                "Failed to read training trajectory file for Modal upload ({}): {}",
                file_path, err
            )
        })?;
        ...
        if !response.status().is_success() {
            ...
            return Err(format!(
                "Modal upload failed (status {}) for {}: {}",
        ...
        loop {
            ...
            if status_changed || message_changed {
                if let Some(fraction) = status.progress_fraction {
                    log_info(format!(
                        "Modal training status: {} ({:.2}%) {}",
                        status.status,
                        fraction * 100.0,
                        message
                    ));
                } else {
                    log_info(format!("Modal training status: {} {}", status.status, message));
                }
            ...
            match status.status.as_str() {
                ...
                "failed" | "cancelled" => {
                    return Err(format!(
                        "Modal training job {} finished with status {}",
                        job_id, status.status
                    ));
                }
                other => {
                    return Err(format!(
                        "Modal training job {} returned unknown status '{}'",
                        job_id, other
                    ));
                }
            }
    ...
    async fn get_status(&self, job_id: &str) -> Result<ModalTrainStatusResponse, String> {
        self.send_json::<(), ModalTrainStatusResponse>(
            reqwest::Method::GET,
...
impl TrainingJobClient for ModalTrainingClient {
    ...
    async fn start_training(
        ...
        ModalTrainingClient::start_training(self, request).await
    }

    async fn upload_trajectory_file(&self, job_id: &str, file_path: &str) -> Result<(), String> {
        ModalTrainingClient::upload_trajectory_file(self, job_id, file_path).await
    }

    async fn wait_until_done(&self, job_id: &str, poll_interval: Duration) -> Result<(), String> {
        ModalTrainingClient::wait_until_done(self, job_id, poll_interval).await
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn modal_client_contract_smoke_test() {
        ...
        Mock::given(method("PUT"))
            .and(path("/train/upload_trajectory/job-123"))
        ...
        Mock::given(method("GET"))
            .and(path("/train/status/job-123"))
            .respond_with(ResponseTemplate::new(200).set_body_json(status_response))
            .mount(&mock_server)
            .await;

        let trajectory_file = NamedTempFile::new().expect("create temp trajectory file");
        std::fs::write(trajectory_file.path(), b"sqlite").expect("write temp trajectory bytes");
        ...
        client
            .upload_trajectory_file(
                &started.job_id,
                trajectory_file.path().to_str().expect("temp path utf8"),
            )
            .await
