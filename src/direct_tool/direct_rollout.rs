...
use crate::{
    direct_tool::{
        direct_tree::DirectTree, direct_tree_action_log::DirectTreeActionLog, hybrid_dataset_entry::HybridDatasetQuestion
    },
...
pub async fn rollout<M: LlmModelMarker>(
    ...
    loop{
        let tree = DirectTree::<M>::from_action_log(
            &action_log,
        ...
        let new_actions = tree.produce_actions_from_direct_tree(&llm_callable, client.clone(), rng).await;
        for action in new_actions {
            action_log.actions.push(action.clone());
        }
        rollout_store.upsert(question.flat_id, &action_log).await.unwrap();
    }
}
